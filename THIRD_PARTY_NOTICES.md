...
Every production build generates `client/public/THIRD_PARTY_NOTICES.txt` from the installed runtime packages and `package-lock.json`. Vite includes it in `dist/client/THIRD_PARTY_NOTICES.txt`, available at `/THIRD_PARTY_NOTICES.txt` in the running app. Preserve that file when distributing a build. It includes upstream notices bundled by Monaco, including notices for its editor font. Source installations also retain each dependency's original license files under `node_modules`.
