...
| AI input and output, only when you enable AI | PR code and metadata, repository conventions for AI Review, Ask questions and conversation history with optional selected-file focus, and optional Jira context, sent by your local Claude CLI using its configured provider and account | Provider retention depends on that account, contract and settings; completed generated results including AI Review share the bounded PR memory cache; Ask history lives in tab memory until navigation, reload or clearing and is not cached by the server |
...
AI is off until you enable it in **Local data & AI**. The disclosure covers repository content and conventions, Ask conversation and focused file context, and optional Jira descriptions sent to the provider configured in Claude. Header insights can start after loading a PR with AI enabled; explanation, checklist and AI Review start when opened, diagrams when requested, and Ask when you submit a question. Enabling AI is a browser preference; turn it off in the same control. The diff and manual review workflow work without AI.
...
Use **Local data & AI** to export saved application data as JSON or clear this app's browser data and server cache. The export may contain private draft text and repository references. It excludes in-memory generated results and Ask conversations. Disabling AI or clearing data cancels active requests and reloads the page; clearing also removes the in-memory chat. Keep it private. Clear affects `pra.*` keys only and does not touch other applications sharing localhost.
