...
import { invalidateSession } from '../state/session.js';
...
export function LocalDataControls() {
  ...
  return <>
    ...
    <dialog ref={dialog} aria-labelledby="local-data-title" className="local-data-dialog">
      ...
      <p>AI sends PR code, descriptions, commit messages, repository conventions, Ask questions and conversation history, the selected file context and optional Jira context through your local Claude account and configured provider. Provider retention and organization policies still apply.</p>
      ...
      <label><input type="checkbox" checked={aiEnabled} onChange={(event) => {
        try {
          ...
          useStore.getState().resetChat();
          invalidateSession();
          // Reload discards all in-memory results after cancelling active requests.
          window.location.reload();
        } catch { setError('This browser cannot save the AI choice. Enable browser storage first.'); }
      }} /> Enable AI for this browser</label>
      <hr />
      <p>Drafts, reviewed files and preferences are saved in this browser until cleared. AI Review results stay in server memory for the cache lifetime. Ask conversations stay in this tab until PR navigation, reload or clearing. They are not included in browser exports. Exports contain private text; store them carefully.</p>
